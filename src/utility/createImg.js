  ...
  createImg: (data) => {
    return new Promise((resolve, reject) => {
      const ind = 50, width = 350, height = 300; // Отступ по краям, длина и высота
        ...
        ctx.stroke();
        const imagePath = path.join(__dirname, '../../public/grafik.jpg');
        const binary = atob(canvas.toDataURL().replace(/^data:image\/png;base64,/, ""));
        fs.writeFile(imagePath, binary, 'binary', function() {
          resolve(imagePath);
        });
